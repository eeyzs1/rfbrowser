  ...
  group('CanvasLayoutService', () {
    ...
    group('computeLayout', () {
      ...
      test('returns empty map for empty cards list', () {
        final result = service.computeLayout([], [], AutoLayoutType.forceDirected);
      ...
      test('switches between layout types', () {
        ...
        final fd = service.computeLayout(cards, connections, AutoLayoutType.forceDirected);
        final hierarchy = service.computeLayout(cards, connections, AutoLayoutType.hierarchical);
        final grid = service.computeLayout(cards, connections, AutoLayoutType.grid);
        expect(fd.length, 3);
      ...
      test('force directed produces positions for all cards', () {
        ...
        final connections = makeChain(cards);
        final result = service.computeLayout(cards, connections, AutoLayoutType.forceDirected, snapToGrid: false);
      ...
      test('force directed positions are finite', () {
        ...
        final result = service.computeLayout(cards, [], AutoLayoutType.forceDirected, snapToGrid: false);
      ...
      test('hierarchical respects direction connections', () {
        ...
        final result = service.computeLayout(cards, connections, AutoLayoutType.hierarchical, snapToGrid: false);
      ...
      test('grid layout arranges cards in rows and columns', () {
        ...
        final result = service.computeLayout(cards, [], AutoLayoutType.grid, snapToGrid: false);
        expect(result.length, 6);
        final sorted = result.entries.toList()..sort((a, b) => a.value.dy.compareTo(b.value.dy));
        expect(sorted[0].value.dy, lessThan(sorted[3].value.dy));
      });

      test('snap to grid rounds positions', () {
        final cards = makeCards(3);
        final result = service.computeLayout(cards, [], AutoLayoutType.grid, snapToGrid: true);
        for (final offset in result.values) {
