...
void main() {
  ...
  group('PluginHostNotifier API Bridge (P0-1 ACs)', () {
    test(
      'AC-IMP-1-1: knowledge.getNote returns real note content (not hardcoded stub)',
      () async {
        ...
        final result = await host._testHandleApiCall('knowledge.getNote', {
          'id': note.filePath,
        });
    ...
    test(
      'AC-IMP-1-1b: knowledge.getNote for non-existent note returns error',
      () async {
        final tempDir = Directory.systemTemp.createTempSync('rfb_pi_');
        addTearDown(() => tempDir.deleteSync(recursive: true));
        ...
        final container = ProviderContainer(
          overrides: [
            noteRepositoryProvider.overrideWith(
              (ref) => NoteRepository(tempDir.path),
            ),
          ],
        );
        addTearDown(container.dispose);
    ...
    test(
      'AC-IMP-1-2: knowledge.search reads from IndexStore (not hardcoded empty list)',
      () async {
        final tempDir = Directory.systemTemp.createTempSync('rfb_pi_');
        addTearDown(() => tempDir.deleteSync(recursive: true));
    ...
    test(
      'AC-IMP-1-3: browser.getCurrentUrl returns real active tab URL',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        final host = container.read(pluginHostProvider.notifier);
        final result = await host._testHandleApiCall(
    ...
    test(
      'AC-IMP-1-3b: browser.getCurrentUrl with no tabs returns empty',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
