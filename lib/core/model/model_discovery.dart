...
class ModelDiscovery {
  ...
  Future<List<AIModel>> _fetchOpenAIModels(AIProvider provider, String? apiKey) async {
  ...
  Future<List<AIModel>> _fetchAnthropicModels(AIProvider provider, String? apiKey) async {
    final headers = <String, String>{
      'Content-Type': 'application/json',
  ...
  Future<List<AIModel>> _fetchOllamaModels(AIProvider provider) async {
    ...
    for (final item in data['models']) {
      ...
      if (details != null) {
        ...
        if (families != null && families.any((f) => f == 'clip' || f == 'llava')) {
  ...
  Set<ModelCapability> _inferCapabilities(String modelId) {
    ...
    final visionKeywords = ['vision', 'visual', '4o', 'gpt-4o', 'gpt-4-turbo', 'claude-3-5', 'claude-3-opus', 'claude-3-sonnet', 'gemini', 'qwen-vl', 'llava', 'pixtral'];
...
final modelDiscoveryProvider = Provider<ModelDiscovery>((ref) => ModelDiscovery());
