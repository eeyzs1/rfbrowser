...
class CanvasPainter extends CustomPainter {
  ...
  CanvasPainter({
    ...
    required this.visibleWorldRect, required this.selectedCardIds, this.connectingFromCardId,
    required this.searchMatchedIds, required this.searchActiveIndex,
    this.connectingPreviewEnd, this.hoveredCardId, this.hoveredConnectionSide,
    this.selectedConnectionId, required this.primaryColor, required this.dividerColor,
    ...
    required this.baseFontSize, this.inlineEditingCardId,
    this.groups = const [], this.layers = const [], this.selectionRect, this.alignmentGuides = const [],
    this.backgroundColorValue, this.rulersVisible = false, this.animationValue = 0,
  });
  ...
  void paint(Canvas canvas, Size size) {
    if (backgroundColorValue != null) {
      canvas.drawRect(Rect.fromLTWH(0, 0, viewW, viewH), Paint()..color = Color(backgroundColorValue!)..style = PaintingStyle.fill);
    }
  ...
  void _drawGroups(Canvas canvas) {
    for (final group in groups) {
      ...
      final groupCards = cards.where((c) => group.cardIds.contains(c.id)).toList();
      ...
      const padding = 16.0;
      final groupRect = Rect.fromLTRB(minX - padding, minY - padding - 20, maxX + padding, maxY + padding);
      final screenRect = Rect.fromLTRB(
        _w2s(groupRect.left, 0).dx, _w2s(0, groupRect.top).dy,
        _w2s(groupRect.right, 0).dx, _w2s(0, groupRect.bottom).dy,
      );
      ...
      final rrect = RRect.fromRectAndRadius(screenRect, const Radius.circular(12));
  ...
  void _drawConnections(Canvas canvas) {
    ...
    void drawLine(CanvasConnection conn, bool forceDashed) {
      ...
      final lineColor = isDashed ? primaryColor.withValues(alpha: 0.5) : Color(connStyle.colorValue);
      ...
      switch (pathType) {
        case ConnectionPath.straight:
          ...
          arrowFrom = conn.waypoints.isNotEmpty ? _w2s(conn.waypoints.last.dx, conn.waypoints.last.dy) : fp;
        case ConnectionPath.orthogonal:
          path = _buildOrthogonalPath(fp, tp, conn.fromSide, conn.toSide, conn.waypoints);
          arrowFrom = conn.waypoints.isNotEmpty ? _w2s(conn.waypoints.last.dx, conn.waypoints.last.dy) : fp;
        case ConnectionPath.curved:
          if (conn.waypoints.isNotEmpty) {
            path = Path()..moveTo(fp.dx, fp.dy);
            final screenWaypoints = conn.waypoints.map((w) => _w2s(w.dx, w.dy)).toList();
            for (int i = 0; i < screenWaypoints.length; i++) {
              final prev = i == 0 ? fp : screenWaypoints[i - 1];
              final curr = screenWaypoints[i];
              final next = i < screenWaypoints.length - 1 ? screenWaypoints[i + 1] : tp;
              final cp1 = Offset(
            ...
            path = Path()..moveTo(fp.dx, fp.dy)..cubicTo(cp1.dx, cp1.dy, cp2.dx, cp2.dy, tp.dx, tp.dy);
            arrowFrom = cp2;
          }
      }

      final paint = Paint()..color = lineColor..strokeWidth = lineW..style = PaintingStyle.stroke;
      if (conn.id == selectedConnectionId) {
        final selPaint = Paint()..color = primaryColor.withValues(alpha: 0.15)..strokeWidth = lineW + 6..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
      ...
      if (connStyle.arrowStyle != ArrowStyle.none && !forceDashed) {
        _drawArrowHead(canvas, arrowFrom, tp, lineColor, connStyle.arrowStyle, connStyle.arrowSize);
      } else if (connStyle.arrowStyle == ArrowStyle.none) {
        // no arrow
      } else if (!isDashed) {
        _drawArrowHead(canvas, arrowFrom, tp, lineColor, ArrowStyle.filledTriangle, connStyle.arrowSize);
      } else {
        _drawArrowHead(canvas, arrowFrom, tp, lineColor, ArrowStyle.triangle, connStyle.arrowSize);
      }

      if (connStyle.startArrowStyle != ArrowStyle.none && !forceDashed) {
        final firstWp = conn.waypoints.isNotEmpty ? _w2s(conn.waypoints.first.dx, conn.waypoints.first.dy) : tp;
        _drawArrowHead(canvas, firstWp, fp, lineColor, connStyle.startArrowStyle, connStyle.arrowSize);
      }
      ...
      if (conn.label.isNotEmpty) {
        ...
        canvas.drawRRect(bgRect, Paint()..color = (isDashed ? hintColor : Color(connStyle.colorValue)).withValues(alpha: 0.3)..style = PaintingStyle.stroke..strokeWidth = 0.5);
      ...
      if (conn.waypoints.isNotEmpty && !forceDashed) {
        ...
        for (final wp in conn.waypoints) {
          final swp = _w2s(wp.dx, wp.dy);
          canvas.drawCircle(swp, wpSize, Paint()..color = scaffoldBg..style = PaintingStyle.fill);
          canvas.drawCircle(swp, wpSize, Paint()..color = lineColor..style = PaintingStyle.stroke..strokeWidth = 1.5);
          canvas.drawCircle(swp, wpSize * 0.375, Paint()..color = lineColor..style = PaintingStyle.fill);
        }
      }
    }

    for (final conn in connections) { drawLine(conn, false); }
    ...
    if (connectingFromCardId != null && connectingPreviewEnd != null) {
      ...
      if (fromCard != null) {
        ...
        canvas.drawPath(path, Paint()..color = primaryColor.withValues(alpha: 0.3)..strokeWidth = 1.5..style = PaintingStyle.stroke);
      }
    }
  }

  Path _buildOrthogonalPath(Offset fp, Offset tp, ConnectionSide fromSide, ConnectionSide toSide, [List<Offset> waypoints = const []]) {
    ...
    Offset p1, p2;
    switch (fromSide) {
      case ConnectionSide.right: p1 = Offset(midX, fp.dy);
      case ConnectionSide.left: p1 = Offset(midX, fp.dy);
      case ConnectionSide.bottom: p1 = Offset(fp.dx, midY);
      case ConnectionSide.top: p1 = Offset(fp.dx, midY);
    }
    switch (toSide) {
      case ConnectionSide.left: p2 = Offset(midX, tp.dy);
      case ConnectionSide.right: p2 = Offset(midX, tp.dy);
      case ConnectionSide.top: p2 = Offset(tp.dx, midY);
      case ConnectionSide.bottom: p2 = Offset(tp.dx, midY);
    }
  ...
  void _drawDashedPath(Canvas canvas, Path path, Paint paint, double dash, double gap, [double offset = 0]) {
  ...
  void _drawArrowHead(Canvas canvas, Offset from, Offset to, Color color, ArrowStyle style, [double arrowSize = 8.0]) {
    ...
    switch (style) {
      ...
      case ArrowStyle.diamond:
        ...
        final back = Offset(to.dx - len * 0.8 * math.cos(angle), to.dy - len * 0.8 * math.sin(angle));
        final path = Path()..moveTo(to.dx, to.dy)..lineTo(mid.dx, mid.dy)..lineTo(back.dx, back.dy)..lineTo(mid2.dx, mid2.dy)..close();
        canvas.drawPath(path, Paint()..color = color..style = PaintingStyle.fill);
      case ArrowStyle.circle:
        canvas.drawCircle(Offset(to.dx - len * 0.4 * math.cos(angle), to.dy - len * 0.4 * math.sin(angle)), len * 0.35, Paint()..color = color..style = PaintingStyle.fill);
    }
  }
  ...
  void _drawCards(Canvas canvas) {
    ...
    for (final card in cards) {
      ...
      else { borderColor = Color(s.borderColor); borderW = s.borderWidth * scale; }

      final cardColor = Color(card.colorValue);
      final rrect = RRect.fromRectAndRadius(cardRect, Radius.circular(effectiveRadius));

      if (s.shadow) {
        canvas.drawShadow(Path()..addRRect(rrect), Colors.black.withValues(alpha: 0.12), 10, false);
        canvas.drawShadow(Path()..addRRect(rrect), Colors.black.withValues(alpha: 0.06), 20, false);
      }
      ...
      if (s.gradientColor != null) {
        ...
        final bgColor = isDark ? cardColor.withValues(alpha: 0.15) : cardColor.withValues(alpha: 0.06);
        canvas.drawRRect(rrect, Paint()..color = bgColor..style = PaintingStyle.fill);
      }

      if (s.opacity < 1.0) {
        canvas.saveLayer(cardRect, Paint()..color = Colors.white.withValues(alpha: s.opacity));
      }

      final highlightPaint = Paint()
        ..shader = LinearGradient(
          begin: Alignment.topCenter, end: Alignment.bottomCenter,
          colors: [Colors.white.withValues(alpha: isDark ? 0.04 : 0.15), Colors.transparent],
          stops: const [0.0, 0.5],
        ).createShader(cardRect);
      canvas.drawRRect(rrect, highlightPaint);

      if (isInlineEditing) {
        canvas.drawRRect(rrect, Paint()..color = primaryColor.withValues(alpha: 0.6)..style = PaintingStyle.stroke..strokeWidth = 1.8);
        canvas.drawRRect(rrect, Paint()..color = primaryColor.withValues(alpha: 0.08)..style = PaintingStyle.stroke..strokeWidth = 4);
      ...
      final headerRect = Rect.fromLTWH(cardRect.left, cardRect.top, cardRect.width, 30 * scale);
      ...
      canvas.drawRRect(headerRRect, Paint()..color = headerBg..style = PaintingStyle.fill);

      final headerGradientPaint = Paint()
        ..shader = LinearGradient(
          begin: Alignment.centerLeft, end: Alignment.centerRight,
          colors: [cardColor.withValues(alpha: isDark ? 0.12 : 0.06), Colors.transparent],
      ...
      final iconPos = Offset(cardRect.left + 10 * scale, cardRect.top + (30 * scale - iconSize) / 2);
      ...
      final titleStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontWeight: FontWeight.w600, fontSize: cardFontSize, color: noteDeleted ? Colors.orange : null, letterSpacing: 0.2);
      ...
      tp.paint(canvas, Offset(cardRect.left + 26 * scale, cardRect.top + (30 * scale - tp.height) / 2));

      final separatorY = cardRect.top + 30 * scale;
      canvas.drawLine(Offset(cardRect.left + 10 * scale, separatorY), Offset(cardRect.right - 10 * scale, separatorY),
        Paint()..color = dividerColor.withValues(alpha: 0.5)..strokeWidth = 0.5);
      ...
      final maxLines = contentHeight > 0 ? (contentHeight / lineHeight).floor().clamp(1, 50) : 1;

      if (displayContent.isNotEmpty) {
        final contentStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontSize: cardFontSize * 1.06, height: 1.4, color: displayContent == 'Empty note' ? hintColor : null);
        final cp = TextPainter(text: TextSpan(text: displayContent, style: contentStyle), textDirection: TextDirection.ltr, maxLines: maxLines, ellipsis: '...');
      ...
      if (displayContent.isEmpty && linkedNote == null) {
        final emptyStyle = TextStyle(color: hintColor.withValues(alpha: 0.7), fontSize: cardFontSize * 1.06, fontStyle: FontStyle.italic);
        final emptyTp = TextPainter(text: TextSpan(text: card.type == CanvasCardType.note ? 'Empty note' : 'Type something...', style: emptyStyle), textDirection: TextDirection.ltr, maxLines: 1);
      ...
      switch (s.borderStyle) {
        ...
        case CardBorderStyle.dashed:
          _drawDashedRRect(canvas, rrect, Paint()..color = borderColor..style = PaintingStyle.stroke..strokeWidth = borderW, 6.0 * scale, 4.0 * scale);
        case CardBorderStyle.dotted:
          _drawDashedRRect(canvas, rrect, Paint()..color = borderColor..style = PaintingStyle.stroke..strokeWidth = borderW, 2.0 * scale, 2.0 * scale);
      ...
      if (!isSelected && s.borderStyle != CardBorderStyle.none) {
        ...
        canvas.drawCircle(Offset(cardRect.right - 7 * scale, cardRect.bottom - 7 * scale), dotR, Paint()..color = hintColor.withValues(alpha: 0.35));
      }
      ...
      if (isSelected) {
        ...
        canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center: Offset(cornerX, cornerY), width: cornerR * 2.5, height: cornerR * 2.5), Radius.circular(cornerR * 0.6)),
          Paint()..color = primaryColor.withValues(alpha: 0.08)..style = PaintingStyle.fill..maskFilter = const MaskFilter.blur(BlurStyle.normal, 4));

        final cornerHandle = RRect.fromRectAndRadius(
          Rect.fromCenter(center: Offset(cornerX, cornerY), width: cornerR * 2, height: cornerR * 2),
          Radius.circular(cornerR * 0.4),
        );
        canvas.drawRRect(cornerHandle, Paint()..color = primaryColor.withValues(alpha: 0.15)..style = PaintingStyle.fill);
        canvas.drawRRect(cornerHandle, Paint()..color = primaryColor.withValues(alpha: 0.65)..style = PaintingStyle.stroke..strokeWidth = 1.2 * scale);

        final gLen = cornerR * 0.45;
        final gOff = cornerR * 0.22;
        final gp = Paint()..color = primaryColor..strokeWidth = 1.3 * scale..style = PaintingStyle.stroke;
        ...
        canvas.drawLine(Offset(gcx - gLen * 0.45, gcy - gLen * 0.45), Offset(gcx, gcy), gp);

        final ew = 4 * scale;
        canvas.drawRect(Rect.fromLTWH(cardRect.right - ew, cardRect.top + 30 * scale, ew, cardRect.height - 30 * scale - handleSize), Paint()..color = primaryColor.withValues(alpha: 0.07));
        canvas.drawRect(Rect.fromLTWH(cardRect.left, cardRect.bottom - ew, cardRect.width - handleSize, ew), Paint()..color = primaryColor.withValues(alpha: 0.07));
      }
    }
  }

  void _drawContainerCard(Canvas canvas, CanvasCard card, Rect clipRect, Map<String, Note> noteMap) {
    final childCards = cards.where((c) => card.childIds.contains(c.id)).toList();
    ...
    final pos = _w2s(card.x, card.y);
    final containerRect = Rect.fromLTWH(pos.dx, pos.dy, containerW * scale, containerH * scale);
    ...
    final rrect = RRect.fromRectAndRadius(containerRect, Radius.circular(effectiveRadius));
    ...
    final headerRect = Rect.fromLTWH(containerRect.left, containerRect.top, containerRect.width, 40 * scale);
    final headerRRect = RRect.fromRectAndCorners(headerRect,
      topLeft: Radius.circular(effectiveRadius),
      topRight: Radius.circular(effectiveRadius));
    ...
    final collapsePaint = Paint()
      ...
      ..style = PaintingStyle.stroke;
    if (card.collapsed) {
      canvas.drawRect(Rect.fromLTWH(collapseX, collapseY, collapseIconSize, collapseIconSize), collapsePaint);
      canvas.drawLine(Offset(collapseX + collapseIconSize / 2, collapseY + 2 * scale), Offset(collapseX + collapseIconSize / 2, collapseY + collapseIconSize - 2 * scale), collapsePaint);
      canvas.drawLine(Offset(collapseX + 2 * scale, collapseY + collapseIconSize / 2), Offset(collapseX + collapseIconSize - 2 * scale, collapseY + collapseIconSize / 2), collapsePaint);
    } else {
      canvas.drawRect(Rect.fromLTWH(collapseX, collapseY, collapseIconSize, collapseIconSize), collapsePaint);
      canvas.drawLine(Offset(collapseX + 2 * scale, collapseY + collapseIconSize / 2), Offset(collapseX + collapseIconSize - 2 * scale, collapseY + collapseIconSize / 2), collapsePaint);
    }
    ...
    tp.paint(canvas, Offset(containerRect.left + 28 * scale, containerRect.top + (40 * scale - tp.height) / 2));

    canvas.drawRRect(rrect, Paint()
      ..color = isSelected ? primaryColor : dividerColor.withValues(alpha: 0.4)
      ..style = PaintingStyle.stroke
  ...
  void _drawSelectionRect(Canvas canvas) {
    ...
    canvas.drawRect(screenRect, Paint()..color = const Color(0xFF3B82F6).withValues(alpha: 0.1)..style = PaintingStyle.fill);
    canvas.drawRect(screenRect, Paint()..color = const Color(0xFF3B82F6).withValues(alpha: 0.5)..style = PaintingStyle.stroke..strokeWidth = 1.0);
  }
  ...
  void _drawCardTypeIcon(Canvas canvas, CanvasCardType type, Offset pos, double size, Paint paint) {
    final path = switch (type) {
      ...
      CanvasCardType.container => Path()
        ...
        ..lineTo(pos.dx + size, pos.dy + size * 0.3),
      CanvasCardType.rectangle => Path()
        ..moveTo(pos.dx, pos.dy)
        ..lineTo(pos.dx + size, pos.dy)
        ..lineTo(pos.dx + size, pos.dy + size)
        ..lineTo(pos.dx, pos.dy + size)
        ..close(),
      CanvasCardType.roundedRect => Path()
        ..addRRect(RRect.fromRectAndRadius(Rect.fromLTWH(pos.dx, pos.dy, size, size), Radius.circular(size * 0.2))),
      CanvasCardType.ellipse => Path()
        ..addOval(Rect.fromLTWH(pos.dx, pos.dy, size, size)),
      CanvasCardType.diamond => Path()
        ..moveTo(pos.dx + size / 2, pos.dy)
        ..lineTo(pos.dx + size, pos.dy + size / 2)
        ..lineTo(pos.dx + size / 2, pos.dy + size)
        ..lineTo(pos.dx, pos.dy + size / 2)
        ..close(),
      CanvasCardType.hexagon => Path()
        ...
        ..lineTo(pos.dx, pos.dy + size / 2)
        ..close(),
      CanvasCardType.parallelogram => Path()
        ..moveTo(pos.dx + size * 0.2, pos.dy)
        ..lineTo(pos.dx + size, pos.dy)
        ..lineTo(pos.dx + size * 0.8, pos.dy + size)
        ..lineTo(pos.dx, pos.dy + size)
        ..close(),
      CanvasCardType.triangle => Path()
        ..moveTo(pos.dx + size / 2, pos.dy)
        ..lineTo(pos.dx + size, pos.dy + size)
        ..lineTo(pos.dx, pos.dy + size)
        ..close(),
      CanvasCardType.cylinder => Path()
        ...
        ..addOval(Rect.fromLTWH(pos.dx, pos.dy + size * 0.7, size, size * 0.3)),
      CanvasCardType.star => Path()
        ...
        ..close(),
      CanvasCardType.swimlaneH || CanvasCardType.swimlaneV => Path()
        ..moveTo(pos.dx, pos.dy)
        ..lineTo(pos.dx + size, pos.dy)
        ..lineTo(pos.dx + size, pos.dy + size)
        ..lineTo(pos.dx, pos.dy + size)
      ...
      CanvasCardType.freehand => Path()
        ..moveTo(pos.dx + size * 0.1, pos.dy + size * 0.5)
        ..quadraticBezierTo(pos.dx + size * 0.3, pos.dy + size * 0.1, pos.dx + size * 0.5, pos.dy + size * 0.5)
        ..quadraticBezierTo(pos.dx + size * 0.7, pos.dy + size * 0.9, pos.dx + size * 0.9, pos.dy + size * 0.4),
    };
  ...
  Path _buildGeometricPath(CanvasCard card, Rect cardRect) {
    ...
    return switch (card.type) {
      ...
      CanvasCardType.roundedRect => Path()..addRRect(RRect.fromRectAndRadius(cardRect, Radius.circular(8.0 * scale))),
      ...
      CanvasCardType.star => Path()
        ...
        ..close(),
      CanvasCardType.table => Path()..addRRect(RRect.fromRectAndRadius(cardRect, Radius.circular(4.0 * scale))),
      CanvasCardType.freehand => Path()..addRect(cardRect),
      _ => Path()..addRRect(RRect.fromRectAndRadius(cardRect, Radius.circular(8.0 * scale))),
    };
  }
  ...
  void _drawGeometricCard(Canvas canvas, CanvasCard card, Rect clipRect) {
    ...
    if (s.shadow) {
      canvas.drawShadow(shapePath, Colors.black.withValues(alpha: 0.12), 8, false);
    }
    ...
    if (s.gradientColor != null) {
      ...
      final bgColor = isDark ? cardColor.withValues(alpha: 0.15) : cardColor.withValues(alpha: 0.06);
      canvas.drawPath(shapePath, Paint()..color = bgColor..style = PaintingStyle.fill);
    }

    final highlightPaint = Paint()
      ..shader = LinearGradient(
        begin: Alignment.topCenter, end: Alignment.bottomCenter,
        colors: [Colors.white.withValues(alpha: isDark ? 0.04 : 0.12), Colors.transparent],
        stops: const [0.0, 0.5],
      ).createShader(cardRect);
    canvas.drawPath(shapePath, highlightPaint);

    final hasContent = card.title.isNotEmpty || card.content.isNotEmpty || card.richContent.isNotEmpty;
    if (hasContent) {
      canvas.save();
      canvas.clipPath(shapePath);
      final cardFontSize = card.effectiveFontSize(baseFontSize);
      final textColor = Color(card.textColorValue);
      final effectiveFontFamily = card.fontFamily.isNotEmpty ? card.fontFamily : null;
      ...
      switch (card.textAlignV) {
        case TextAlignV.top: textY = cardRect.top + 8 * scale;
        case TextAlignV.middle: textY = cardRect.center.dy - 20 * scale;
        case TextAlignV.bottom: textY = cardRect.bottom - 40 * scale;
      }

      if (card.title.isNotEmpty) {
        final titleStyle = (bodySmallStyle ?? const TextStyle()).copyWith(
          fontWeight: FontWeight.w600, fontSize: cardFontSize,
      ...
      if (card.richContent.isNotEmpty) {
        final baseStyle = (bodySmallStyle ?? const TextStyle()).copyWith(
          fontSize: cardFontSize * 0.9, height: 1.3,
        ...
        final spans = card.richContent.map((seg) {
          final style = switch (seg.type) {
            RichTextSegmentType.bold => baseStyle.copyWith(fontWeight: FontWeight.bold),
            RichTextSegmentType.italic => baseStyle.copyWith(fontStyle: FontStyle.italic),
            RichTextSegmentType.underline => baseStyle.copyWith(decoration: TextDecoration.underline),
            RichTextSegmentType.code => baseStyle.copyWith(fontFamily: 'monospace', backgroundColor: isDark ? Colors.white10 : Colors.black.withValues(alpha: 0.05)),
            RichTextSegmentType.strikethrough => baseStyle.copyWith(decoration: TextDecoration.lineThrough),
        ...
        final cp = TextPainter(
          ...
          textDirection: TextDirection.ltr, maxLines: 5, ellipsis: '...',
          textAlign: textAlign,
        );
        ...
        final cp = TextPainter(
          ...
          textDirection: TextDirection.ltr, maxLines: 3, ellipsis: '...',
          textAlign: textAlign,
        );
        cp.layout(maxWidth: maxTextWidth);
        final offsetX = card.textAlignH == TextAlignH.center
    ...
    if (card.tags.isNotEmpty) {
      ...
      for (final tag in card.tags.take(3)) {
        final tagStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontSize: (baseFontSize * 0.55).clamp(6.0, 9.0), color: primaryColor.withValues(alpha: 0.8));
        final tp = TextPainter(text: TextSpan(text: '#$tag', style: tagStyle), textDirection: TextDirection.ltr, maxLines: 1);
        tp.layout();
        final tagBg = RRect.fromRectAndRadius(Rect.fromLTWH(tagX, tagY, tp.width + 6 * scale, 12 * scale), Radius.circular(3 * scale));
        canvas.drawRRect(tagBg, Paint()..color = primaryColor.withValues(alpha: 0.08)..style = PaintingStyle.fill);
    ...
    Color borderColor; double borderW;
    if (isSelected) { borderColor = primaryColor; borderW = 2; }
    else { borderColor = Color(s.borderColor); borderW = s.borderWidth * scale; }

    switch (s.borderStyle) {
      case CardBorderStyle.solid:
        canvas.drawPath(shapePath, Paint()..color = borderColor..style = PaintingStyle.stroke..strokeWidth = borderW);
      case CardBorderStyle.dashed:
        _drawDashedPath(canvas, shapePath, Paint()..color = borderColor..style = PaintingStyle.stroke..strokeWidth = borderW, 6.0 * scale, 4.0 * scale);
      case CardBorderStyle.dotted:
        _drawDashedPath(canvas, shapePath, Paint()..color = borderColor..style = PaintingStyle.stroke..strokeWidth = borderW, 2.0 * scale, 2.0 * scale);
      case CardBorderStyle.none:
        break;
    }
    ...
    if (isSelected) {
      ...
      for (final h in handles) {
        canvas.drawRect(Rect.fromCenter(center: h, width: handleSize, height: handleSize),
          Paint()..color = scaffoldBg..style = PaintingStyle.fill);
        canvas.drawRect(Rect.fromCenter(center: h, width: handleSize, height: handleSize),
          Paint()..color = primaryColor..style = PaintingStyle.stroke..strokeWidth = 1);
      }
    }
  }
  ...
  void _drawSwimlaneCard(Canvas canvas, CanvasCard card, Rect clipRect, Map<String, Note> noteMap) {
    ...
    final rrect = RRect.fromRectAndRadius(cardRect, Radius.circular(6.0 * scale));
    canvas.drawRRect(rrect, Paint()..color = cardColor.withValues(alpha: isDark ? 0.06 : 0.03)..style = PaintingStyle.fill);

    final headerRect = Rect.fromLTWH(cardRect.left, cardRect.top, cardRect.width, headerH);
    final headerRRect = RRect.fromRectAndCorners(headerRect,
      topLeft: Radius.circular(6.0 * scale),
      topRight: Radius.circular(6.0 * scale));
    canvas.drawRRect(headerRRect, Paint()..color = cardColor.withValues(alpha: isDark ? 0.15 : 0.08)..style = PaintingStyle.fill);

    if (card.title.isNotEmpty) {
      final titleStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontWeight: FontWeight.w600, fontSize: baseFontSize * 0.8, color: cardColor.withValues(alpha: 0.8));
      final tp = TextPainter(text: TextSpan(text: card.title, style: titleStyle), textDirection: TextDirection.ltr, maxLines: 1, ellipsis: '...');
      tp.layout(maxWidth: cardRect.width - 16 * scale);
      tp.paint(canvas, Offset(cardRect.left + 8 * scale, cardRect.top + (headerH - tp.height) / 2));
    }
    ...
    if (card.type == CanvasCardType.swimlaneH) {
      ...
      for (int i = 1; i < laneCount; i++) {
        ...
        canvas.drawLine(Offset(x, cardRect.top + headerH), Offset(x, cardRect.bottom),
          Paint()..color = dividerColor.withValues(alpha: 0.3)..strokeWidth = 1);
      }
    } else {
      ...
      for (int i = 1; i < laneCount; i++) {
        ...
        canvas.drawLine(Offset(cardRect.left, y), Offset(cardRect.right, y),
          Paint()..color = dividerColor.withValues(alpha: 0.3)..strokeWidth = 1);
      }
    }
  ...
  void _drawLineJumps(Canvas canvas, Path path, CanvasConnectionStyle style, String currentConnId) {
    ...
    for (final otherConn in allConns) {
      ...
      final fp = _w2s(otherConn.fromSide.point(from.rect, otherConn.fromSideOffset).dx, otherConn.fromSide.point(from.rect, otherConn.fromSideOffset).dy);
      final tp = _w2s(otherConn.toSide.point(to.rect, otherConn.toSideOffset).dx, otherConn.toSide.point(to.rect, otherConn.toSideOffset).dy);
      final otherStyle = otherConn.style ?? CanvasConnectionStyle.defaults;
      final otherPathType = otherConn.isAuto ? ConnectionPath.curved : otherStyle.pathType;
      Path otherPath;
      switch (otherPathType) {
        case ConnectionPath.straight:
          otherPath = Path()..moveTo(fp.dx, fp.dy)..lineTo(tp.dx, tp.dy);
        case ConnectionPath.orthogonal:
          otherPath = _buildOrthogonalPath(fp, tp, otherConn.fromSide, otherConn.toSide, otherConn.waypoints);
        case ConnectionPath.curved:
          final dx = (tp.dx - fp.dx).abs();
          final dy = (tp.dy - fp.dy).abs();
          final cp = math.max(dx, dy) * 0.4;
          ...
          otherPath = Path()..moveTo(fp.dx, fp.dy)..cubicTo(cp1.dx, cp1.dy, cp2.dx, cp2.dy, tp.dx, tp.dy);
      }
      ...
      for (final otherPath in otherPaths) {
        ...
        for (final om in otherMetrics) {
          for (double od = 0; od < om.length; od += step) {
            ...
            if (ot == null) continue;
            final dist = (point.dx - ot.position.dx) * (point.dx - ot.position.dx) +
            ...
            if (dist < jumpSize * jumpSize * 0.25) {
              switch (style.lineJumpStyle) {
                case LineJumpStyle.arc:
                  final arcRect = Rect.fromCenter(center: point, width: jumpSize * 2, height: jumpSize * 2);
                  canvas.drawArc(arcRect, -math.pi, math.pi, false, jumpBgPaint);
                ...
                case LineJumpStyle.square:
                  canvas.drawRect(Rect.fromCenter(center: point, width: jumpSize, height: jumpSize), jumpBgPaint);
                  final sqPath = Path()
                    ..moveTo(point.dx - jumpSize * 0.5, point.dy)
                    ..lineTo(point.dx - jumpSize * 0.5, point.dy - jumpSize * 0.5)
                    ..lineTo(point.dx + jumpSize * 0.5, point.dy - jumpSize * 0.5)
  ...
  void _drawTableCard(Canvas canvas, CanvasCard card, Rect clipRect) {
    ...
    if (s.shadow) {
      canvas.drawRect(cardRect.translate(2, 2), Paint()..color = Colors.black.withValues(alpha: 0.08)..style = PaintingStyle.fill);
    }
    canvas.drawRect(cardRect, Paint()..color = cardColor.withValues(alpha: isDark ? 0.15 : 0.06)..style = PaintingStyle.fill);

    final headerH = 28.0 * scale;
    canvas.drawRect(Rect.fromLTWH(cardRect.left, cardRect.top, cardRect.width, headerH),
      Paint()..color = cardColor.withValues(alpha: isDark ? 0.2 : 0.1)..style = PaintingStyle.fill);

    if (card.title.isNotEmpty) {
      final titleStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontWeight: FontWeight.w600, fontSize: baseFontSize * 0.75, color: isDark ? Colors.white70 : Colors.black87);
      final tp = TextPainter(text: TextSpan(text: card.title, style: titleStyle), textDirection: TextDirection.ltr, maxLines: 1, ellipsis: '...');
      tp.layout(maxWidth: cardRect.width - 16 * scale);
      tp.paint(canvas, Offset(cardRect.left + 8 * scale, cardRect.top + (headerH - tp.height) / 2));
    }
    ...
    final linePaint = Paint()..color = dividerColor.withValues(alpha: 0.3)..strokeWidth = 0.5;

    for (int c = 1; c < cols; c++) {
      final x = cardRect.left + cellW * c;
      canvas.drawLine(Offset(x, cardRect.top + headerH), Offset(x, cardRect.bottom), linePaint);
    }
    for (int r = 0; r < rows; r++) {
      final y = cardRect.top + headerH + cellH * r;
      canvas.drawLine(Offset(cardRect.left, y), Offset(cardRect.right, y), linePaint);
      for (int c = 0; c < cols; c++) {
        final idx = r * cols + c;
        if (idx < card.tableCells.length && card.tableCells[idx].text.isNotEmpty) {
          final cellStyle = (bodySmallStyle ?? const TextStyle()).copyWith(fontSize: baseFontSize * 0.65, color: isDark ? Colors.white60 : Colors.black54);
          final cp = TextPainter(text: TextSpan(text: card.tableCells[idx].text, style: cellStyle), textDirection: TextDirection.ltr, maxLines: 1, ellipsis: '...');
          cp.layout(maxWidth: cellW - 6 * scale);
          cp.paint(canvas, Offset(cardRect.left + cellW * c + 3 * scale, y + (cellH - cp.height) / 2));
        }
      }
    }
    ...
    canvas.drawRect(cardRect, Paint()
      ..color = isSelected ? primaryColor : dividerColor.withValues(alpha: 0.4)
      ..style = PaintingStyle.stroke..strokeWidth = isSelected ? 2 : 1);
  }
  ...
  void _drawFreehandCard(Canvas canvas, CanvasCard card, Rect clipRect) {
    ...
    if (isSelected) {
      ...
      canvas.drawRect(bounds.inflate(4), Paint()..color = primaryColor.withValues(alpha: 0.2)..style = PaintingStyle.fill);
      canvas.drawRect(bounds.inflate(4), Paint()..color = primaryColor..style = PaintingStyle.stroke..strokeWidth = 1);
    }
  }
  ...
  void _drawConnectionPoints(Canvas canvas) {
    ...
    for (final card in cards) {
      ...
      final showDots = selectedCardIds.contains(card.id) ||
      ...
      void drawDot(double sx, double sy, ConnectionSide side) {
        final isHovered = card.id == hoveredCardId && side == hoveredConnectionSide;
        final r = isHovered ? hoverDotRadius : dotRadius;
        final color = isHovered ? hoverColor : dotColor;
        canvas.drawCircle(Offset(sx, sy), r, Paint()..color = color..style = PaintingStyle.fill);
        canvas.drawCircle(Offset(sx, sy), r, Paint()..color = dotBorder..style = PaintingStyle.stroke..strokeWidth = 1);
      }
    ...
    switch (style.flowAnimation) {
      case FlowAnimationStyle.flow:
        ...
        for (int i = 0; i < dotCount; i++) {
          ...
          if (tangent != null) {
            ...
            canvas.drawCircle(pos, 5.0 * scale, Paint()..color = baseColor.withValues(alpha: 0.15)..style = PaintingStyle.fill);
            canvas.drawCircle(pos, 3.0 * scale, Paint()..color = baseColor.withValues(alpha: 0.8)..style = PaintingStyle.fill);
            canvas.drawCircle(pos, 1.5 * scale, Paint()..color = Colors.white.withValues(alpha: 0.6)..style = PaintingStyle.fill);
          }
        }
      ...
      case FlowAnimationStyle.dash:
        ...
        _drawDashedPath(canvas, path, Paint()
          ...
          ..style = PaintingStyle.stroke
          ..strokeWidth = style.strokeWidth + 1, dashLen, gapLen, offset);
  ...
  void _drawRulers(Canvas canvas) {
    ...
    final rulerColor = isDark ? Colors.white.withValues(alpha: 0.06) : Colors.black.withValues(alpha: 0.04);
    final tickColor = isDark ? Colors.white.withValues(alpha: 0.3) : Colors.black.withValues(alpha: 0.3);
    final textColor = isDark ? Colors.white.withValues(alpha: 0.5) : Colors.black.withValues(alpha: 0.5);

    canvas.drawRect(Rect.fromLTWH(0, 0, viewW, rulerWidth), Paint()..color = rulerColor..style = PaintingStyle.fill);
    canvas.drawRect(Rect.fromLTWH(0, 0, rulerWidth, viewH), Paint()..color = rulerColor..style = PaintingStyle.fill);
    canvas.drawRect(Rect.fromLTWH(0, 0, rulerWidth, rulerWidth), Paint()..color = rulerColor..style = PaintingStyle.fill);

    final tickInterval = gridSize * scale;
    final majorEvery = 5;
    final tickPaint = Paint()..color = tickColor..strokeWidth = 0.5;
    ...
    for (double x = startX; x < viewW; x += tickInterval) {
      ...
      canvas.drawLine(Offset(x, rulerWidth - tickH), Offset(x, rulerWidth), tickPaint);
      if (isMajor) {
        final worldX = cameraX + (x - viewW / 2) / scale;
        final label = '${worldX.round()}';
        final tp = TextPainter(text: TextSpan(text: label, style: TextStyle(fontSize: 7, color: textColor)), textDirection: TextDirection.ltr);
        tp.layout();
    ...
    for (double y = startY; y < viewH; y += tickInterval) {
      ...
      canvas.drawLine(Offset(rulerWidth - tickW, y), Offset(rulerWidth, y), tickPaint);
      if (isMajor) {
    ...
    canvas.drawLine(Offset(rulerWidth, 0), Offset(rulerWidth, viewH), Paint()..color = tickColor..strokeWidth = 0.5);
    canvas.drawLine(Offset(0, rulerWidth), Offset(viewW, rulerWidth), Paint()..color = tickColor..strokeWidth = 0.5);
  }
