...
mixin _CanvasPanelsMixin on _CanvasViewStateBase {
  ...
  Widget _buildActiveFilterBanner(ThemeData theme, CanvasData canvasData) {
    ...
    if (canvasData.selectedLayerId == null) return const SizedBox.shrink();

    String label;
    if (canvasData.selectedLayerId == CanvasData.unassignedSentinel) {
      label = '${l.tagActiveFilter}: ${l.tagFilterUnassigned}';
    } else {
      final layer = canvasData.layers
          .where((ly) => ly.id == canvasData.selectedLayerId)
          .firstOrNull;
      label =
          '${l.tagActiveFilter}: ${layer?.name ?? canvasData.selectedLayerId}';
    }

    return Positioned(
      top: _CanvasViewStateBase._toolbarHeight + 8,
      left: 0,
      right: 0,
      child: Center(
  ...
  Widget _buildMinimap(ThemeData theme, CanvasData canvasData) {
    final visibleCards = canvasData.cards.where((c) {
